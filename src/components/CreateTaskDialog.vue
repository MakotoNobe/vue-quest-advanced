     ...
     <v-card>
         ...
         <v-card-text>
                    <v-container>
                        <v-row>
                            <v-col cols="12">
                                <v-text-field v-model="eventName" label="タイトル" required></v-text-field>
                            </v-col>
                            <v-col cols="6"> 
                                <v-menu v-model="startMenu" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" min-width="auto">
                            ...
                            <v-col cols="6">
                                <v-menu
                                    ...
                                    >  
                                    <template v-slot:activator="{on ,attrs}">
                                    <v-text-field v-model="endDate" label="終了日" prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                    </template>
                                        <v-date-picker
                                        v-model="endDate"
                                        @input="endMenu = false"
                            ...
                            <template v-slot:activator="{on ,attrs}">
                                <v-btn :color="eventLabelColor" dark rounded v-bind="attrs" v-on="on">
                                    ラベルの色
                                </v-btn>
                            </template>
                    ...
                    <v-col cols="12">
                    <v-text-field v-model="eventDetail" label="詳細"></v-text-field>
                    </v-col>
                </v-row>
                </v-container>
            </v-card-text>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="red darken-1" text @click="closeDailog">
          閉じる
          </v-btn>
        <v-btn color="blue darken-1" text @click="createEvent" :disabled="inputCheck">
          作成
          </v-btn>
      </v-card-actions>
    </v-card>
